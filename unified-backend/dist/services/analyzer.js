...
function analyzePoliticalContext(text) {
    const content = text.toLowerCase();
    const detectedParties = [];
    const regionSpecific = [];
    const biasIndicators = [];
    const factCheckWarnings = [];
...
async function analyzeDeepfakePatterns(buffer, mimetype) {
...
function analyzeMisinformationPatterns(text) {
    const content = text.toLowerCase();
    const propagandaPatterns = [];
    const factualInconsistencies = [];
    const emotionalManipulation = [];
    const sourcingIssues = [];
    const narrativeConstruction = [];
...
async function analyzeWithGPT4Vision(buffer, mimetype) {
...
Text: "${text.substring(0, 1200)}"`
        ...
        return {
            ...
            meta: { model, tokensUsed: result.usage?.total_tokens || 0 }
        };
    }
    catch (error) {
...
Provide your analysis as JSON:
{
  ...
  "overallAssessment": "comprehensive evaluation with confidence explanation"
        ...
        if (claudeAnalysis.cognitiveMarkers && Array.isArray(claudeAnalysis.cognitiveMarkers)) {
            flags.push(...claudeAnalysis.cognitiveMarkers.map((marker) => `Human: ${marker}`));
        }
        ...
        try {
            ...
            if (Array.isArray(result) && result.length > 0) {
                ...
                if (Array.isArray(predictions)) {
                    const aiLabel = predictions.find((p) => ['AI', 'FAKE', 'machine', 'ai-generated'].some(label => p.label?.toLowerCase().includes(label.toLowerCase())));
                    if (aiLabel) {
                ...
                // For sequence classification
                    ...
                    const aiIndex = predictions.labels.findIndex((label) => ['ai-generated', 'FAKE', 'AI'].includes(label));
                    if (aiIndex !== -1) {
...
async function analyzeImageWithHuggingFace(buffer) {
    const huggingFaceApiKey = process.env.HUGGING_FACE_API_KEY;
    if (!huggingFaceApiKey)
        return null;
    ...
    for (const model of models) {
        try {
            ...
            if (model === "CAI/ai-image-detector" && Array.isArray(result) && result.length > 0) {
                ...
                const predictions = result[0];
                const fakeLabel = predictions.find((p) => p.label === 'fake');
                if (fakeLabel) {
...
async function analyzeLinkEnhanced(url) {
    let content = '';
    let contentType = '';
    let contentLength = 0;
    try {
...
async function analyzeWithAllProviders(text) {
    const results = [];
    const providerNames = [];
...
export async function analyzeIndiaElectionContent(text) {
...
function aggregateResults(results, providerNames) {
...
function analyzeFileHeuristic(buffer, mimetype, originalName, sizeBytes) {
    const isImage = mimetype.startsWith('image/');
    const isVideo = mimetype.startsWith('video/');
    ...
    if (isImage)
        flags.push('Image artifact analysis');
    if (isVideo)
        flags.push('Video frame analysis');
    return {
        confidence,
        flags,
        summary: confidence > 70
...
export async function analyzeFileBuffer(buffer, mimetype, originalName, sizeBytes) {
...
export { analyzePoliticalContext, analyzeDeepfakePatterns, analyzeMisinformationPatterns };
