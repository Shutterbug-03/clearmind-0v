...
function calculateThreatLevel(political: any, misinformation: any): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
  let score = 0;
  
  if (political.isElectionRelated) score += 2;
  if (political.detectedParties.length > 2) score += 2;
  if (political.biasIndicators.length > 2) score += 3;
  if (misinformation.propagandaPatterns.length > 2) score += 4;
  if (misinformation.emotionalManipulation.length > 2) score += 3;
  if (misinformation.sourcingIssues.length > 1) score += 2;
...
function calculateThreatScore(political: any, misinformation: any): number {
  return Math.min(
    (political.biasIndicators.length * 10) +
    (misinformation.propagandaPatterns.length * 15) +
    (misinformation.emotionalManipulation.length * 12) +
    (political.detectedParties.length > 2 ? 20 : 0), 
    100
  );
}

function identifyPrimaryRisks(political: any, misinformation: any): string[] {
  const risks = [];
...
function generateSecurityRecommendations(threatLevel: string): string[] {
  switch (threatLevel) {
    case 'CRITICAL':
      return [
